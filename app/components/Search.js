...
function Search(){
    ...
    //listen to the ESC keyboard to remove the search component
    useEffect(() => {
      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results " + (state.show == "results" ? "live-search-results--visible" : '')}>
            {Boolean(state.results.length) && (
              ...
              <div className="list-group-item active"><strong>Search Results</strong> ({state.results.length} { state.results.length > 1 ? 'Items' : 'Item'} found)</div>
              {
                state.results.map( post => {
                  ...
                  return <Post post = {post} id={post._id} onClick={ ()=> {appDispatch({type: "closeSearch"})} }/>
            ...
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-ms">Sorry, no results found for your search</p>}
          </div>
        </div>
      </div>
    </div>
